      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white p-3 rounded-[0.7rem] border border-[#00000080] space-y-1">
          ...
          <p className="font-semibold text-sm">
            ...
            <button
            className="text-[10px] text-primary px-2 py-0.5 rounded-full mt-1 flex gap-3 border border-primary items-center"
            onClick={() => setShowPayWithCardModal(true)}
            >
            Top Up wallet
            <DashboardRight />
            </button>
            {showPayWithCardModal && token && (
    ...
    userEmail={userEmail}
      ...
      <div className="space-y-1 p-2 bg-[#f9f9f9] rounded">
        <div className="flex justify-between items-center">
          <span className="text-xs text-[#00000080] font-medium">
            Total Ad Spend
          </span>
